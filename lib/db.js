...
async function ensureSchema() {
  ...
  // Migración: agregar numero_empleado (el número con el que la persona está
  // dada de alta en la nómina de GPA). Es TEXT a propósito: hay números que
  // podrían llevar ceros a la izquierda o algún prefijo, y nunca se hacen
  // operaciones aritméticas con él. No lleva restricción de unicidad porque
  // un mismo empleado puede tener dos usuarios en la app (uno como
  // supervisor, que califica, y otro como operativo, para ser evaluado):
  // ambos comparten legítimamente el mismo número de empleado.
  await sql`
    ALTER TABLE personal
  ...
  if (rows[0].count === 0) {
    for (const p of PERSONAL_SEED) {
      await sql`
        INSERT INTO personal (id, nombre, puesto, sucursal, sucursal_code, role, username, password, activo, fecha_alta, numero_empleado)
        VALUES (${p.id}, ${p.nombre}, ${p.puesto}, ${p.sucursal}, ${p.sucursal_code}, ${p.role}, ${p.username || null}, ${p.password || null}, true, '2020-01-01', ${p.numero_empleado || null})
